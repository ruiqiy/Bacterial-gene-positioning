...
REVISION_ROOT <- if (length(script_arg)) {
  script_dir <- dirname(normalizePath(
    sub("^--file=", "", script_arg[[1]]),
    winslash = "/"
  ))
  if (basename(script_dir) == "scripts") dirname(script_dir) else script_dir
} else {
  candidate <- normalizePath(getwd(), winslash = "/")
  if (basename(candidate) == "Revision_1") candidate else file.path(candidate, "Revision_1")
}
PROJECT_ROOT <- dirname(REVISION_ROOT)
ACTIVE_GENE_SET <- Sys.getenv("GENE_SET_FILTER", unset = "all")
VALID_GENE_SETS <- c(
  "all",
  "core",
  "accessory",
...
if (!ACTIVE_GENE_SET %in% VALID_GENE_SETS) {
  stop(
    "GENE_SET_FILTER must be one of: ",
    paste(VALID_GENE_SETS, collapse = ", ")
  )
}
ANALYSIS_OUTPUT_ROOT <- Sys.getenv(
  "GENE_SET_OUTPUT_ROOT",
  unset = file.path(
    REVISION_ROOT,
    "gene_set_analysis",
    ACTIVE_GENE_SET
...
setwd(ANALYSIS_OUTPUT_ROOT)

# Discard any graphics drawn without an explicitly opened device. All retained
# figures below use pdf() or ggsave(), so this prevents stray empty Rplots.pdf
# files without changing named analysis outputs.
...
# ===============================================
# Define file mapping
RAW_JOINED_ROOT <- file.path(
  REVISION_ROOT,
  "data",
  "analysis_ready_proteomics"
)
file_list <- c(
  schmidt_2016 = file.path(
    RAW_JOINED_ROOT,
    "schmidt_2016_joined.rds"
  ),
  peebo_2015 = file.path(
    RAW_JOINED_ROOT,
    "peebo_2015_joined.rds"
  ),
  li_2014_all = file.path(
    RAW_JOINED_ROOT,
    "li_2014_all_joined.rds"
  ),
  valgepea_2013_prot = file.path(
    RAW_JOINED_ROOT,
    "valgepea_2013_prot_joined.rds"
  ),
  Goelzer_2015 = file.path(
    RAW_JOINED_ROOT,
    "Goelzer_2015_joined.rds"
  ),
  zhu_B_subtilis = file.path(
    RAW_JOINED_ROOT,
    "zhu_B_subtilis_joined.rds"
  ),
  zhu_E_coli = file.path(
    RAW_JOINED_ROOT,
    "zhu_E_coli_joined.rds"
  ),
  zhu_V_natriegens = file.path(
    RAW_JOINED_ROOT,
    "zhu_V_natriegens_joined.rds"
  )
)
...
DATASETS <- purrr::imap(
  ...
  function(data, dataset_name) {
    ...
    if (!all(required %in% names(data))) {
      stop(
        dataset_name,
        ": joined RDS is missing identifier-pipeline columns: ",
        paste(setdiff(required, names(data)), collapse = ", ")
      )
    }
    if (any(!data$analysis_eligible)) {
      stop(dataset_name, ": ineligible identifier mappings reached analysis")
    }
    if (anyDuplicated(data$reference_identifier)) {
      stop(dataset_name, ": duplicated reference identifiers reached analysis")
    }
    if (!identical(
      as.character(data$gene),
      as.character(data$reference_identifier)
    )) {
      stop(dataset_name, ": gene is not the stable reference identifier")
    }
    if (any(!is.finite(data$norm_pos))) {
      stop(dataset_name, ": reference position is missing or nonfinite")
    }
    data
  }
)
...
# Apply one gene-set universe before any gene statistics, consensus models, or
# main figures are calculated. Core/accessory use the main PanKB mapping;
# unresolved memberships are excluded from the corresponding subset.
gene_set_membership_mask <- function(data, gene_set) {
  if (gene_set == "all") return(rep(TRUE, nrow(data)))
  if (gene_set == "core") {
    return(data$binary_core_accessory_main == "core")
  }
  if (gene_set == "accessory") {
    return(data$binary_core_accessory_main == "accessory")
  }
  if (gene_set == "transcription_translation") {
    return(data$is_transcription_translation %in% TRUE)
  }
  data$is_other_gene %in% TRUE
}

DATASETS <- purrr::imap(
  DATASETS,
  function(data, dataset_name) {
    membership <- gene_set_membership_mask(data, ACTIVE_GENE_SET)
    membership[is.na(membership)] <- FALSE
    filtered <- data[membership, , drop = FALSE]
    message(
      "Gene-set filter: ",
      ACTIVE_GENE_SET,
      " | ",
...
# E. coli and B. subtilis calls below require presence in 3 and 2 datasets,
# respectively. The interaction analysis is the only unfiltered exception.
...
res_bsub  <- run_consensus_pipeline(bsub_ds, "B. subtilis Consensus", min_datasets = 2)
...
# ==============================================================================
if (ENABLE_DOWNSAMPLING && ACTIVE_GENE_SET == "all") {
...
ds_res_ecoli <- run_downsampling_sim(
  ecoli_ds, "E. coli Consensus", step_size = 5,
  min_datasets_req = 3
)

# B. subtilis: 2 dataset minimum (Matches 'run_consensus_pipeline' call)
ds_res_bsub  <- run_downsampling_sim(
  bsub_ds, "B. subtilis Consensus", step_size = 1,
  min_datasets_req = 2
)
...
if (!is.null(all_sim_res) && nrow(all_sim_res) > 0) {
  ...
  if (!ENABLE_DOWNSAMPLING) {
    ...
    message(
      "\nDownsampling analysis is skipped for gene set: ",
      ACTIVE_GENE_SET,
      ". It runs only for the 'all' gene set."
    )
  }
}
# ===============================================
...
if (
  file.exists(file.path(REVISION_ROOT, "data", "promoter", "mapped", "LB_promoter_identifiers.csv")) &&
    file.exists(file.path(REVISION_ROOT, "data", "promoter", "mapped", "M9_promoter_identifiers.csv"))
) {
  
  # -------------------------------------------------------
  # 1. Build Consensus Dataset (Matched to Deleted Section Logic)
  # -------------------------------------------------------
  process_species_consensus_internal <- function(ds_list, min_datasets) {
  ...
  ecoli_consensus <- process_species_consensus_internal(ecoli_ds, 3) %>%
    mutate(gene = tolower(trimws(gene)))

  # Promoter observations only require a mapped gene position; they do not
  # need to occur in the >=3-dataset expression consensus.
  ecoli_gene_positions <- read_csv(
    file.path(REVISION_ROOT, "data", "reference_gene_positions.csv"),
    show_col_types = FALSE
  ) %>%
    filter(species == "Escherichia coli", is.finite(norm_pos)) %>%
    transmute(
      gene = tolower(trimws(reference_identifier)),
      norm_pos
    )

  if (ACTIVE_GENE_SET != "all") {
    eligible_ecoli_genes <- map_dfr(
      DATASETS_FULL[ecoli_ds],
      ~ transmute(.x, gene = tolower(trimws(gene)))
    ) %>%
      distinct(gene)
    ecoli_gene_positions <- semi_join(
      ecoli_gene_positions, eligible_ecoli_genes, by = "gene"
    )
  }
  ...
  # -------------------------------------------------------
  read_identifier_mapped_promoters <- function(path, medium_name) {
    mapped <- read_csv(path, show_col_types = FALSE) %>%
      mutate(
        gene = tolower(trimws(reference_identifier)),
        medium = medium_name
      )
    message(
      medium_name,
      " promoter direct reference-ID coverage: ",
      sum(!is.na(mapped$gene)),
      "/",
      nrow(mapped)
    )
    mapped %>% filter(!is.na(gene))
  }

  lb_raw <- read_identifier_mapped_promoters(
    file.path(REVISION_ROOT, "data", "promoter", "mapped", "LB_promoter_identifiers.csv"),
    "LB"
  ) %>%
  ...
  m9_raw <- read_identifier_mapped_promoters(
    file.path(REVISION_ROOT, "data", "promoter", "mapped", "M9_promoter_identifiers.csv"),
    "M9"
  ) %>%
    select(gene, start = left_boundary_coordinate, end = right_boundary_coordinate, 
           peak = peak_coordinate, activity = peak_activity, id = name)
  ...
  # -------------------------------------------------------
  # 4. Attach gene positions and calculate moving averages
  # -------------------------------------------------------
  merged_data <- ecoli_gene_positions %>%
  ...
  cor_stats_prom <- merged_data %>%
    ...
    summarise(
      ...
      Pearson_r = cor(Value, norm_pos, method = "pearson", use = "complete.obs"),
      Pearson_p_value = cor.test(Value, norm_pos, method = "pearson", use = "complete.obs")$p.value,
      n_genes = n(),
      .groups = "drop"
    ) 
  ...
  message("Identifier-mapped promoter CSV files missing.")
}

# Requested five-gene-set extension. The original all-gene pipeline above is
# retained; this standalone module adds stratified models, formal interactions,
# sensitivity analyses, QC counts, and supplementary figures.
if (ACTIVE_GENE_SET == "all") {
  source(file.path(REVISION_ROOT, "scripts", "run_gene_set_analyses.R"))
} else {
  message(
    "Skipped the supplementary five-set module because this run already ",
    "uses the single gene-set universe: ",
    ACTIVE_GENE_SET
  )
}
